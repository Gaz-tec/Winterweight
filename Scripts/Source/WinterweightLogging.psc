ScriptName WinterweightLogging
{
A collection of global functions for logging and assertions.
Modified for WinterWeight to use our Domain.
}
...
* obj is a JContainers structure.
	...
	if f3
		WinterweightLogging.Log4(prefix, func, Namer(f1), Namer(f2), Namer(f3), LuaS("", obj))
	elseif f2
		WinterweightLogging.Log3(prefix, func, Namer(f1), Namer(f2), LuaS("", obj))
	elseif f1
		WinterweightLogging.Log2(prefix, func, Namer(f1), LuaS("", obj))
	else
		WinterweightLogging.Log1(prefix, func, LuaS("", obj))
	endIf
EndFunction
...
{ Pretty-prints a JLua object as a string. }
	if name == ""
		return "(" + luaObj + ") =" + JLua_evalLuaStr("return WinterweightLogging.tableToString(args)", luaObj, "NIL", false)
	else
		return name + "(" + luaObj + ") =" + JLua_evalLuaStr("return WinterweightLogging.tableToString(args)", luaObj, "NIL", false)
	endIf
EndFunction
